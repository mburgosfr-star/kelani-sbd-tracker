...
(async () => {
  const chrome = spawn(CHROME, [
    ...
    `--user-data-dir=/tmp/kelani-always-easy-profile-${Date.now()}`,
    APP_URL,
  ], { stdio: 'ignore' });
    ...
    const out = await send('Runtime.evaluate', {
      expression: `(() => {
        ...
        const setDone = workout => ({
          ...
          sets: (workout.sets || []).map(set => ({ ...set, done: true, failed: false, skipped: false, setEffort: 'easy' })),
          lifts: (workout.lifts || []).map(lift => ({
            ...lift,
            prepItems: (lift.prepItems || []).map(item => ({ ...item, done: true })),
            warmups: (lift.warmups || []).map(item => ({ ...item, done: true })),
            sets: (lift.sets || []).map(set => ({ ...set, done: true, failed: false, skipped: false, setEffort: 'easy' })),
          })),
        });
        ...
        return {
          scenario: 'cycle-start-always-easy-no-fails',
          qaPass: qa?.pass,
          meetDay,
          finalState: {
