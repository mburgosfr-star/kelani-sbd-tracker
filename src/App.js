...
function getAchievedHistoryMaxCandidates(entry = {}) {
  if (
    !entry ||
...
const SMART_SECONDARY_EXPOSURE_WEIGHT = 0.5;
...
export function generateWarmups(workPlan, lift = '') {
  ...
  function repsForWarmup(weight, isFinalWarmup) {
    ...
    if (normalizedLift === 'Deadlift' && weight <= 70 && !isFinalWarmup) return 5;
...
function getSmartLiftSetsFromSnapshot(snapshot = {}, lift = null) {
  if (!lift) return [];
...
function buildSmartReadinessSignals(context = {}) {
  ...
  const rollingTrainingDays = workoutDays
    .filter(day =>
      !day.restDay &&
      day.smartDayType !== SMART_DAY_TYPES.RECOVERY &&
      day.smartDayType !== SMART_DAY_TYPES.DELOAD &&
    ...
    .slice(-SMART_THRESHOLDS.ROLLING_TRAINING_DAYS);
...
function getProjectedSmartLiftEligibility({
  ...
  const underTargetPrimaryLifts = LIFT_ORDER
    .filter(lift =>
      Number(weightedExposureCounts[lift]) <
  ...
  const nonRepeatedFallbackPrimaryLifts = LIFT_ORDER
    .filter(lift => lift !== lastPrimaryLift)
    .sort(comparePrimaryLoad);
...
function buildGeneratedSmartTrainingWorkout({
  ...
  const primaryEligibleSet = new Set(
    frequencyEligibility.primaryEligibleLifts
  );
  const secondaryEligibleSet = new Set(
    ...
    smartTrainingSelectionSummary: {
      ...
      reasonFlags: [
        ...
        frequencyEligibility.lastPrimaryLift &&
          ...
          primaryLift !== frequencyEligibility.lastPrimaryLift
...
export function getSmartModalDetailRows(workout = {}, t = {}) {
  ...
  if (
    isFatigueRecovery ||
    (isTrainingFallback && (readiness.meetdayBlockers || []).includes('fatigue'))
  ) {
    const fatigueScore = Number(readiness.recentFatigueScore) || 0;
    ...
    if (lastEffort) {
      rows.push({
        ...
        value: `Previous workout ${lastEffort}`,
      });
    }
