...
// `npx cap sync android` and is gitignored - it does not exist in a
// pristine checkout before sync runs. Callers must only invoke this after
// their own `cap sync` step, not from the unit test suite (which runs
// before sync in every pipeline: CI, test-izzy-build.js, build-release-apk.js).
function assertCapacitorConfigSynced(base = root) {
  const synced = readJson(
    path.join(base, 'android/app/src/main/assets/capacitor.config.json')
  );

  if (synced.appId !== packageName) {
    fail(
      `Synced Capacitor config appId is ${synced.appId}, ` +
      `expected ${packageName}. Run npx cap sync android.`
    );
  }
}
